...
begin
  randomize();
  sauvage := geekedex[random(18)+1];             // recupération du geekemon dans le geekedex
  sortie := false;
  obtenir := false;
  ...
  begin
    ...
    if obtenir = true then  //Si la capture réussi, stock dans le pc
    begin
  ...
  begin
    ...
    begin
      ...
      writeln(perso.pseudo,' utilise une potion');                                   // Utilisation de potion en fonction du nombre possédé
  ...
  if choix = 5 then
  begin
  changement := ChangementGkmnCombat(perso,true);   //Echange le geekemon actuelle avec un autre selectionner dans l'équipe
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        writeln(sauvage.nom,' utilise sont attaque élémentaire');
  ...
  choisirSalle(perso,adversaire,geekedex);  //Fin du combat retour à la selection de la salle
end;
...
begin
  if id < 10 then niveauAdv(adversaire[id],perso)       //Comme l'id 10 correspond au boss on vérifie d'abord si on se bat contre le boss ou pas
  else bossFinal(adversaire[id]);
  adversaireTemp := adversaire[id];   //On crée une copie de l'adversaire
  sortie := false;
  obtenir := false ;
  autorisation := true;
  changement := false;
  ...
  begin
    ...
    restant:= restant - 1;   // On vérifie si l'adversaire possède toujours des geekemons vivant
     if restant = 0 then
     begin
     writeln('Le combat est terminé');
  ...
  begin
    ...
    begin
      ...
      begin
        writeln(perso.equipe[1].nom,' est mort !');
        readln;
        if EquipeEnVie(perso.equipe) then ChangementGkmnCombat(perso,false)  //Vérification si le joueur possède des geekemons vivant
        else
        begin
		   	writeln('Le combat est terminé');       //Fin de combat si équipe morte
			  readln;
			  sortie := true;
        end;
      end;
